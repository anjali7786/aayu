...
shelf_life_analyst = LlmAgent(
    ...
    instruction=(
        ...
        "  1. Call `predict-shelf-life` to get the model's numeric prediction.\n"
        "  2. Interpret the predicted hours against the exposure summary.\n"
        "Return a structured analysis as JSON with these fields:\n"
        "  - predicted_remaining_hours:  float\n"
        "  - ground_truth_remaining_hours: float\n"
...
decision_agent = LlmAgent(
    ...
    instruction=(
        ...
        "Action rules:\n"
        "  - 'Sell Normally' when predicted > 60% of nominal AND no significant excursion.\n"
        "  - 'Prioritize Sale' when 30-60% of nominal AND no significant excursion.\n"
        "  - 'Discount' when 15-30% of nominal.\n"
        "  - 'Inspect' when excursion peak > 5°C over max and remaining life > 30% of nominal.\n"
        "  - 'Quarantine' when remaining life < 15% of nominal OR excursion peak > 8°C over max."
    ),
    output_key="decision",
)
